<script lang="ts">
     ...
     * @type {Array<object>}
     */
     let {listJsonObject} = $props();
     /**
     * @type {String}
     */
     
    function elementHTMLToPDF() {
        dataToPdf(listJsonObject)        
    }
</script>


<button class="w-1/2 h-full m-2 p-2 bg-blue-700 font-semibold text-white rounded-md flex items-center justify-center hover:bg-blue-800" 
onclick={elementHTMLToPDF} title="Gerar PDF">
