<script lang="ts">
    ...
    let countTotalCoverages = $state(0);
    let isProcessing = $state(false);

    async function processCoveragesRequested(coverages: IWCSCoverageDescription[]) {
        countTotalCoverages += coverages.length;
        countWCSProcessado += 1;
        const keywords = coverages
            .flatMap(cov => cov.keywords)
    ...
    async function processRequestForSelectedItems(objIdDescricaoIri: IdDescricaoIRI) {
        try {
            ...
            const wcsCaps = capabilities(xmlText);
            const coverages = wcsCaps.summary?.coverages || [];
            await processCoveragesRequested(coverages);
        } catch (error) {
            console.error('Failed to process WCS for', objIdDescricaoIri.descricao, error);
        }
    };
    ...
    function newObjIdDescricaoIRI(obj: IGeoservicoDescricao, idx: number) {
        return { id: idx, descricao: obj.descricao, iri: obj.wcsGetCapabilities}
    };
...
<form class="m-2">
    <div class="flex flex-col md:flex-row items-center mb-1 text-sm font-medium text-gray-900 dark:text-gray-400">
        ...
        <p class="mr-2"> Quantidade de catálogos processados: {countWCSProcessado}/{selectedItems.length} </p>
        {#if isProcessing}
            <div class="animate-pulse mr-2 text-blue-600 font-semibold">Processando catálogos...</div>
        {/if}
        <p class="ml-auto text-sm">Qtd de coberturas: {countTotalCoverages}</p>
