<script lang="ts">
    ...
    const wfsProtocolFilter = '<Filter xmlns="http://www.opengis.net/ogc"><PropertyIsEqualTo><PropertyName>protocol</PropertyName><Literal>OGC:WFS</Literal></PropertyIsEqualTo></Filter>';

    let totalMetadataCount = $state(0);
    let wfsMetadataCount = $state(0);
    ...
    async function btnCatalogoClicked() {
        ...
        try {
            ...
            wfsMetadataCount = await fetchMatchedRecords(buildGetRecordsUrl(urlCatalogo, wfsProtocolFilter));
        } catch (error) {
            console.error('Erro na requisição em protocolo-wfs-quantidade', error);
...
<div class="mt-4 flex relative text-gray-700">
    <input
...
<p class="mt-2 text-lg text-blue-600">Quantidade de metadados com o protocolo WFS: {wfsMetadataCount}</p>
